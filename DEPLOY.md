...
- `schema.sql` — таблиці `messages`, `digests`, `people`, `polls`, `pics`. Оновлюєш існуючу базу з версії до 25.09 — один раз виконай `npx wrangler d1 execute babtsya --remote --command "ALTER TABLE pics ADD COLUMN data TEXT"`, потім `schema.sql` (він створює лише відсутні таблиці);
